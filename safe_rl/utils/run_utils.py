from safe_rl.user_config import DEFAULT_DATA_DIR, FORCE_DATESTAMP, \
                               DEFAULT_SHORTHAND, WAIT_BEFORE_LAUNCH
from safe_rl.utils.logx import colorize
from safe_rl.utils.mpi_tools import mpi_fork, msg
...
def setup_logger_kwargs(exp_name, seed=None, data_dir=None, datestamp=False):
    ...
    Args:
        ...
        data_dir (string): Path to folder where results should be saved.
            Default is the ``DEFAULT_DATA_DIR`` in ``safe_rl/user_config.py``.
...
def call_experiment(exp_name, thunk, seed=0, num_cpu=1, data_dir=None, 
    ...
    Args:
        ...
        data_dir (string): Used in configuring the logger, to decide where
            ...
            default to ``DEFAULT_DATA_DIR`` from ``safe_rl/user_config.py``. 
    ...
    plot_cmd = 'python -m safe_rl.run plot '+logger_kwargs['output_dir']
    plot_cmd = colorize(plot_cmd, 'green')

    test_cmd = 'python -m safe_rl.run test_policy '+logger_kwargs['output_dir']
...
class ExperimentGrid:
    ...
    def add(self, key, vals, shorthand=None, in_name=False):
        ...
        ``safe_rl/user_config.py`` file to ``False``. 
